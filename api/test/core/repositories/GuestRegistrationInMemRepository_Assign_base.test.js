...
test('should save new assignment if all repository constraints are ok', () => {
    repo.save_assign(assign);
    expect(repo.size_assign()).toBe(1);
})

test('should return all assignments if no parameters passed to load', () => {
    repo.save_assign(assign);
    let assignment_list = repo.load_assign();
...
test('should return correct assignment after saving', () => {
    repo.save_assign(assign);
    let load_assign = repo.load_assign(
...
test('should remove correct assignment after saving', () => {
    repo.save_assign(assign);
    expect(repo.size_assign()).toBe(1);
    repo.remove_assign(assign);
    expect(repo.size_assign()).toBe(0);
});